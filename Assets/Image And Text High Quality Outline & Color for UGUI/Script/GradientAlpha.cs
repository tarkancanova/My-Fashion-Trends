...
{
    //image Gradient Alpha by adamman
    [AddComponentMenu("UI/Adamman/Gradient Alpha"), RequireComponent(typeof(Graphic))]
    public class GradientAlpha : BaseMeshEffect
    {
        private const int ONE_TEXT_VERTEX = 6; // one text vertex has 6 vertex

        [SerializeField, Range(0f, 1f)]  // 0f = transparent, 1f = opaque
        private float m_alphaTop = 1f;  // top alpha
        [SerializeField, Range(0f, 1f)]
        private float m_alphaBottom = 1f; // bottom alpha
        [SerializeField, Range(0f, 1f)]
        private float m_alphaLeft = 1f;  // alpha left
        [SerializeField, Range(0f, 1f)]
        private float m_alphaRight = 1f; // alpha right
